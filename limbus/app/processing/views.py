...
def ProcessingProtocolView(protocol_id) -> dict:
    protocol = db.session.query(ProcessingTemplate).filter(ProcessingTemplate.id == protocol_id).first_or_404()
    ...
    for _, document in db.session.query(ProcessingTemplateToDocument, Document).filter(ProcessingTemplateToDocument.template_id == protocol_id).filter(Document.id == ProcessingTemplateToDocument.document_id).all():
