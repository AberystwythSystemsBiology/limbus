...
def _generate_user_info():
    for row in read_csv("/limbus/demo-data/user.csv"):
        user = User(email=row[0], password="password", is_admin=bool(row[1]))
...
def _generate_sample_attribute() -> None:
    ...
    for row in read_csv("/limbus/demo-data/sampleattribute.csv"):
        ...
        sampleattr = SampleAttribute(
            term=row[0], type=row[1], required=False, author_id=aid
        )
        ...
        if row[1] == "TEXT":
            ...
            setting = SampleAttributeTextSetting(
                max_length=row[2], sample_attribute_id=sampleattr.id
            )
        ...
        if row[1] == "OPTION":
            for option in row[2].split(";"):
                attropt = SampleAttributeOption(
                    term=option, author_id=aid, sample_attribute_id=sampleattr.id
                )
    ...
    for i in range(75):
        ...
        if sample_attr.type == SampleAttributeTypes.OPTION:
            sample_attr_opts = (
                ...
                .filter(SampleAttributeOption.sample_attribute_id == sample_attr.id)
                .all()
            )
            ...
            if (
                len(
                    db.session.query(SampleAttributeOptionValue, SampleAttributeOption)
                    .filter(SampleAttributeOptionValue.sample_id == sample_choice.id)
                    .filter(
            ...
            if (
                len(
                    db.session.query(SampleAttributeTextValue)
                    .filter(SampleAttributeTextValue.sample_id == sample_choice.id)
                    .filter(
