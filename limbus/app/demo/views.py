...
def _generate_sample_data() -> None:
    ...
    for row in read_csv("/limbus/demo-data/sample.csv"):
        sample = Sample(sample_type=row[0],
                        sample_status=row[3],
...
def _generate_sample_attribute() -> None:
    ...
    for row in read_csv("/limbus/demo-data/sampleattribute.csv"):
        ...
        sampleattr = SampleAttribute(term=row[0],
                                     type=row[1],
                                     required=False,
                                     author_id=aid)
...
def _associate_sample_attributes() -> None:
    ...
    for i in range(75):
        ...
        if sample_attr.type == SampleAttributeTypes.OPTION:
            ...
            if len(
                    db.session.query(
                        ...
                        SampleAttributeOption).filter(
                            SampleAttributeOptionValue.sample_id ==
                            sample_choice.id).filter(
                                SampleAttributeOptionValue.sample_attribute_id
                                == sample_attr.id).all()) == 0:
            ...
            if len(
                    db.session.query(SampleAttributeTextValue).filter(
                        SampleAttributeTextValue.sample_id == sample_choice.id
                    ).filter(SampleAttributeTextValue.sample_attribute_id ==
                             sample_attr.id).all()) == 0:
