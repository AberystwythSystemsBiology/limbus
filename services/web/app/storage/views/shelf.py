...
def ShelfView(shelf_id: int) -> dict:
    ...
    boxes = db.session.query(CryovialBox).join(CryovialBoxToFixedColdStorageShelf).filter(CryovialBoxToFixedColdStorageShelf.shelf_id == shelf_id).all()
    samples = db.session.query(SampleToFixedColdStorageShelf).filter(SampleToFixedColdStorageShelf.shelf_id == shelf_id).join(FixedColdStorageShelf).all()
