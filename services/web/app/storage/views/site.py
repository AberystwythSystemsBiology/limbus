...
def SiteView(site_id: int) -> dict:
    ...
    address = db.session.query(Address).filter(Address.id == data["address_id"]).first_or_404()
    ...
    data["rooms"] = [BasicRoomView(x.id) for x in db.session.query(Room).filter(Room.site_id == site_id).all()]
