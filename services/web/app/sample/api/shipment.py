...
def add_samples_to_cart(tokenuser: UserAccount):
    ...
    if samples_locked.count() > 0:
        ...
        msg_locked = " | ".join(
            ["LIMBSMP-%s" % (sample.id) for sample in samples_locked]
        )
    else:
        ids_locked = []
        msg_locked = []
...
def add_samples_with_rack_to_cart(tokenuser: UserAccount):
    ...
    samples_locked = db.session.query(Sample.id).filter(
        Sample.id.in_(sample_ids), Sample.is_locked is True
    )
    ...
    if samples_locked.count() > 0:
        ...
        msg_locked = " | ".join(
            ["LIMBSMP-%s" % (sample_id) for sample_id in samples_locked]
        )
    else:
        ids_locked = []
        msg_locked = []
